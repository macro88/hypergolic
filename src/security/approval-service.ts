...
export interface NativeApprovalLeasePort {
  ...
  finish(lease: NativeApprovalLease): void;
}
...
export type ApprovalSnapshot = Readonly<{ focusedSessionId: string | null; reviewPaused: boolean; current: ApprovalItem | null; pending: number }>;
type Entry = ApprovalItem & { lease: NativeApprovalLease; controller: AbortController | null };
...
export class ApprovalService {
  ...
  constructor(native: NativeApprovalLeasePort, owner: TrustedApprovalOwner, effects: ApprovalEffects) {
    ...
    this.snapshot = Object.freeze({ focusedSessionId: null, reviewPaused: false, current: null, pending: 0 });
  }
  ...
  private publish() {
    ...
    const next = Object.freeze({ focusedSessionId: this.focusedSessionId, reviewPaused: this.reviewPaused, current: shown, pending: this.entries.length });
    if (next.focusedSessionId === this.snapshot.focusedSessionId && next.reviewPaused === this.snapshot.reviewPaused && next.pending === this.snapshot.pending && next.current?.id === this.snapshot.current?.id) return;
    this.snapshot = next;
    for (const listener of this.listeners) try { listener(); } catch { /* UI observers never hold approval authority. */ }
  }
  private end(entry: Entry, cancel: boolean) {
    ...
    try { this.native.finish(entry.lease); } catch { /* Terminal native cleanup. */ }
  }
  refresh() {
    for (const entry of this.entries) { try { if (!this.native.isLive(entry.lease)) this.end(entry, false); } catch { this.end(entry, true); } }
    this.publish();
  }
  enqueue(token: string, destinationsFromTrustedConfig: unknown): boolean {
    this.refresh();
    ...
    try {
      ...
      this.entries.push({ id: `approval_${++nextPresentationId}`, origin: captureOrigin(taken.origin), event, lease: taken.lease, controller: null });
  ...
  resume() { if (this.foregrounded && this.focusedSessionId !== null) this.reviewPaused = false; this.publish(); }
  dismiss(id: string) {
    this.refresh();
  ...
  async approve(id: string): Promise<readonly RelayOutcome[]> {
    ...
    try {
      ...
      execution.assertActive();
      return captureOutcomes(outcomes, entry.event.destinations, verified.id);
    } finally { this.end(entry, true); this.publish(); }
  }
}
