...
Last activity: 2026-09-15 — Implemented strict identity/storage broker, native-held immutable request snapshots and one-use synchronous lease checks in both hosts. All 22 real-SQLite broker tests and 100 assertions on each native-language registry pass. Android prebuild and Kotlin/Java module compilation pass. The iPhone Simulator passes all 17 source-linked WK boundary tests in one run, including four transport/source/revocation cases; sealed inputs/products remain identical. Full aislop is 100/100 and full local React Doctor has zero findings; numeric scoring remains pending. The application owner and Kehto fixture runtime are not yet connected to native storage. See docs/native-capabilities.md. Progress metadata counts plans, not product completion.
